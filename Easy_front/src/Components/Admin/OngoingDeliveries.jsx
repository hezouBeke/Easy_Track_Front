        ...
        <div className="pt-4 space-y-3" style={{ maxHeight: '550px', overflowY: 'auto', scrollbarWidth: 'none' }}>
          {expeditions.map((expedition, index) => (
            ...
            >
              ...
              <h3 className="text-md font-semibold text-black">{expedition.expedition_code}</h3>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex justify-between items-center mt-2 text-xs">
                <div className="text-gray-600">
                  <p>Départ</p>
                  <p className="font-semibold">{new Date(expedition.date_depart).toLocaleString()}</p>
                </div>
                <div className="text-gray-600">
                  <p>Arrivée</p>
                  <p className="font-semibold">{new Date(expedition.date_arrivee).toLocaleString()}</p>
                </div>
              </div>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex justify-between items-center mt-2 text-xs">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-semibold">{expedition.colis?.client_id_exp || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">NumColis</p>
                  <p className="font-semibold">{expedition.colis?.numero_colis || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">Description</p>
                  <p className="font-semibold">{expedition.colis?.description || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">Poids</p>
                  <p className="font-semibold">{expedition.colis?.poids ? `${expedition.colis.poids} kg` : 'N/A'}</p>
                </div>
              </div>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex items-center mt-4">
                ...
                <div className="text-xs">
                  ...
                  {/* Vérifier si "course" existe avant d'y accéder */}
                  <p className="font-semibold">{expedition.course?.coursier_id || 'N/A'}</p>
                </div>
