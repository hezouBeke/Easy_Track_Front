  ...
  useEffect(() => {
    const fetchExpeditions = async () => {
      try {
        ...
        console.log(response.data); // Loguer les données pour vérifier leur structure
    ...
    <div className="p-3 bg-white text-black rounded-lg shadow-lg mt-1 w-full">
      ...
      <div className="pt-4 space-y-3" style={{ maxHeight: '550px', overflowY: 'auto', scrollbarWidth: 'none' }}>
        {expeditions.map((expedition, index) => {
          const firstCourse = expedition.course_ids?.[0]; // Récupère le premier coursier
          const coursierActuel = firstCourse?.coursier_id?.completename || 'N/A';
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="px-2 py-1 bg-yellow-400 text-white text-xs rounded-full">{expedition?.status || 'In Transit'}</span>
              </div>
              <h3 className="text-md font-semibold text-black">{expedition?.expedition_code || 'N/A'}</h3>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex justify-between items-center mt-2 text-xs">
                <div className="text-gray-600">
                  <p>Départ</p>
                  <p className="font-semibold">{new Date(expedition?.date_depart).toLocaleString() || 'N/A'}</p>
                </div>
                <div className="text-gray-600">
                  <p>Arrivée</p>
                  <p className="font-semibold">{new Date(expedition?.date_arrivee).toLocaleString() || 'N/A'}</p>
                </div>
              </div>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex justify-between items-center mt-2 text-xs">
                {/* Vérifier si "colis" existe avant d'y accéder */}
                <div className="flex-1">
                  <p className="text-gray-600">Client</p>
                  <p className="font-semibold">{expedition?.colis_id?.client_id_exp?.completename || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">NumColis</p>
                  <p className="font-semibold">{expedition?.colis_id?.numero_colis || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">Description</p>
                  <p className="font-semibold">{expedition?.colis_id?.description || 'N/A'}</p>
                </div>
                <div className="flex-1">
                  <p className="text-gray-600">Poids</p>
                  <p className="font-semibold">{expedition?.colis_id?.poids ? `${expedition.colis_id.poids} kg` : 'N/A'}</p>
                </div>
              </div>
              <hr className="my-2 border-gray-300" style={{ borderWidth: '0.5px' }} />
              <div className="flex items-center mt-4">
                ...
                <div className="text-xs">
                  ...
                  <p className="font-semibold">{coursierActuel}</p>
                </div>
